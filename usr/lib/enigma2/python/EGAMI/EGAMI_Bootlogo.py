...
class EgamiBootLogoSelector(Screen):
    ...
    def keyGo(self):
        if self['config'].getCurrent() is not None:
            self.session.openWithCallback(self.confirm, MessageBox, _('Would You like to use this bootlogo as default?'), MessageBox.TYPE_YESNO, timeout=10, default=False)
        return
